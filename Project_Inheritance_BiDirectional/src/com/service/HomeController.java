...
public class HomeController implements controller {
 Scanner sc=new Scanner(System.in);
	...
	public void dcreate(Session session) {
		...
		d.setDid(sc.nextInt());
		System.out.println("Enter Device Type: Laptop Or Mobile");
		d.setDtype(sc.next()+sc.nextLine());
		session.save(d);
		session.beginTransaction().commit();
	...
	public void ddelete(Session session) {
		System.out.println("Enter Device Id to remove from our database");
		Device d = session.get(Device.class, sc.nextInt());
		session.delete(d);
		session.beginTransaction().commit();
